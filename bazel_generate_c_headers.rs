...
pub fn generate_c_headers(
    buggy_headers_path: String,
    splitguide_yaml: String,
    version_txt: String,
    zenoh_commons_h: String,
    zenoh_concrete_h: String,
    zenoh_configure_h: String,
    zenoh_macros_h: String,
    zenoh_opaque_h: String,
) {
    const GENERATION_PATH: &str = "include/zenoh-gen.h";
    const PREPROCESS_PATH: &str = "include/zenoh-cpp.h";
    fix_cbindgen(buggy_headers_path, GENERATION_PATH);
    preprocess_header(GENERATION_PATH, PREPROCESS_PATH);
    create_generics_header(PREPROCESS_PATH, zenoh_macros_h);
    configure(version_txt, zenoh_configure_h);
    let files = split_bindings(splitguide_yaml, GENERATION_PATH).unwrap();
